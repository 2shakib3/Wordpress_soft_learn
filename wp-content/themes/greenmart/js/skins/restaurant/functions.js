(function($) {
    ...
    var back_to_top_restaurant = function () {
        jQuery(window).scroll(function () {
            if (jQuery(this).scrollTop() > 550) {

                if( jQuery('.fixed').hasClass('active') ) return;
    ...
    var back_to_top_restaurant = function () {
        jQuery(window).scroll(function () {
            if (jQuery(this).scrollTop() > 400) {
                jQuery('.social-fixed-left').addClass('active');
            } else {
                jQuery('.social-fixed-left').removeClass('active');
            }
        });
    };

    back_to_top_restaurant();
    ...
    jQuery( document).ready( function($){
        ...
        $(".search-horizontal .btn-search-totop").on( "click", function() {
            $( ".search-horizontal" ).toggleClass('active');
        });
        ...
        function greenmart_click_horizontal_search() {
            ...
            if ( $box_search.has(event.target).length == 0 && !$box_search.is(event.target) ){
                    $( ".search-horizontal" ).removeClass('active');
                }
            });
        } 
